 ...
 * @experimental
 */
export const ResolveObjectAttributes = Symbol.for("@virtualstate/kdl/resolveObjectAttributes");
 ...
 */
export function toKDLString(input: unknown, options?: ToKDLStringOptions): TheAsyncThing<string>
export function toKDLString(input: unknown, options?: unknown): TheAsyncThing<string>
export function toKDLString(input: unknown, options?: unknown): TheAsyncThing<string>{
    return anAsyncThing(toKDLStringInternal(input, options));
}

async function *toKDLStringInternal(input: unknown, options?: unknown): AsyncIterable<string> {
    let yielded = false;
    // const {
    //     name: nameInput,
    //     props,
    ...
    const propsObjects = propsEntries.filter(([, value]) => isUnknownJSXNode(value));
    const propsObjectsStrings = new Map(option(ResolveObjectAttributes) && propsObjects.length ? (
        await Promise.all(
            propsObjects.map(
                async ([key, object]) => [key, `{${(await toKDLString(object)).trim()}}`] as const
            )
        )
    ) : []);

    const propsString = Object.keys(props).map(key => `${key}=${propsObjectsStrings.get(key) ?? JSON.stringify(props[key])}`).join(" ");
