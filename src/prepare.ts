...
export function prepare(node: unknown, queryInput: string) {
  ...
  function part(query: QueryToken[], result = root): Split<unknown> {
    ...
    if (isDirectChildToken(token)) {
      ...
      if (
        follows &&
        (isImmediatelyFollowsToken(follows) || isFollowsToken(follows))
      ) {
        afterNext = rest;
    ...
    if (isTopToken(token)) {
      ...
      function isFollowsMatch(
        ...
        if (isImmediatelyFollowsToken(follows)) {
          return isMatch(before.at(-1), left);
        }

        const foundIndex = before.findIndex((node) => isMatch(node, left));
        return foundIndex > -1;
      }
    ...
    const unmatchResult = unmatch.flatMap((node) =>
      part(query, children(node))
    );
    if (!rest.length) {
...
function getFromAccessor(node: unknown, token: AccessorToken): unknown {
  ...
  if (left && right && op) {
    ok(isOperation(op));
    return operator(access(node, left), access(node, right), op);
  }
...
function splitAt<T extends { type: string }>(
  array: T[],
  fn: (value: T) => boolean
): T[][] {
  const result: T[][] = [];
