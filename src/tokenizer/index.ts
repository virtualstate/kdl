...
export type QueryValueToken = BooleanToken | NumberToken | StringToken;
...
export function isQueryValueToken(token: QueryToken): token is QueryValueToken {
  return isStringToken(token) || isNumberToken(token) || isBooleanToken(token);
}
...
export function isGetPropertyToken(
  token: QueryToken
): token is GetPropertyToken {
...
export function isDirectChildToken(
  token: QueryToken
): token is DirectChildToken {
...
export function isImmediatelyFollowsToken(
  token: QueryToken
): token is ImmediatelyFollowsToken {
...
export function* query(value: string): Iterable<QueryToken> {
  ...
  for (const token of tokens) {
    if (token.tokenType === Query.Selector) {
      ...
      const {
        tokens: [left, operator, right, ...rest],
  ...
  function getValue(token?: Chevrotain.IToken): QueryToken | undefined {
    ...
    if (
      token.tokenType === Query.String ||
      token.tokenType === QueryAccessor.String
    ) {
      return {
        type: "String",
    ...
    if (
      token.tokenType === Query.Raw ||
      token.tokenType === QueryAccessor.Raw
    ) {
      return {
        type: "String",
    ...
    if (
      token.tokenType === Query.Number ||
      token.tokenType === QueryAccessor.Number
    ) {
      return {
        type: "Number",
        number: +token.image,
    ...
    if (
      token.tokenType === Query.Boolean ||
      token.tokenType === QueryAccessor.Boolean
    ) {
      return {
        type: "Boolean",
    ...
    if (token.tokenType === QueryAccessor.GetValue) {
      ...
      return {
        type: "GetValue",
        index:
          typeof index === "string" && /^\d+$/.test(index) ? +index : undefined,
        image: token.image,
      };
    }
